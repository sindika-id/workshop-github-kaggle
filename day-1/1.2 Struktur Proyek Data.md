...
Struktur berikut direkomendasikan untuk proyek data yang dikerjakan secara kolaboratif dan berjangka panjang. Struktur ini mengadopsi praktik terbaik dari **Cookiecutter Data Science**.
...
## Referensi: Cookiecutter Data Science

Struktur ini terinspirasi dari standar berikut:

🔗 https://cookiecutter-data-science.drivendata.org/

Cookiecutter Data Science adalah konvensi struktur proyek data yang bertujuan agar proyek:

- Mudah dipahami
- Mudah direproduksi
- Mudah dikolaborasikan
- Mudah dikembangkan

Prinsip utamanya adalah pemisahan yang jelas antara **data**, **kode**, **eksperimen**, dan **output**.

---

## Struktur Proyek Minimal (Direkomendasikan untuk Pemula)

Struktur berikut sudah cukup untuk sebagian besar proyek Machine Learning sederhana:
...
Struktur ini mencerminkan workflow inti:
load data → eksplorasi → training → evaluasi.

---

## Komponen Required (Wajib / Sangat Disarankan)

| Komponen | Keterangan |
|--------|-----------|
| README.md | Menjelaskan tujuan proyek, dataset, dan cara menjalankan kode |
| requirements.txt | Menjamin environment dapat direplikasi |
| data/raw/ | Menyimpan dataset asli atau petunjuk pengambilan data |
| src/ | Menyimpan source code utama proyek |
| src/data/ | Logika pemuatan data |
| src/models/ | Training dan evaluasi model |
| src/utils/seed.py | Menjaga reproducibility eksperimen |
| notebooks/ | Sangat dianjurkan untuk eksplorasi data |

---

## Komponen Nice-to-Have (Opsional / Bertahap)

| Komponen | Digunakan Ketika |
|--------|------------------|
| LICENSE | Proyek dipublikasikan atau open-source |
| .gitignore | Menghindari file tidak penting masuk repository |
| .env.example | Proyek menggunakan environment variable |
| data/interim/ | Ada preprocessing bertahap |
| data/processed/ | Dataset final digunakan berulang |
| src/config/ | Parameter mulai banyak |
| src/pipelines/ | Training terorkestrasi |
| experiments/ | Banyak eksperimen |
| models_artifacts/ | Model disimpan |
| reports/ | Dokumentasi hasil |
| scripts/ | Automasi training |

---

## Prinsip Penting

1. Mulai dari yang minimal
2. Tambahkan struktur berdasarkan kebutuhan nyata
3. Struktur proyek bersifat dinamis
4. Fokus utama adalah workflow, bukan kerapian folder
